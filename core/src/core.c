...
#ifdef ENABLE_ONVM
		//onvm_nflib_stop(NULL);  //I need to open it test
#endif
...
/*----------------------------------------------------------------------------*/
#ifdef TIME_STAT
struct stat_counter dbg_pkt_proc, dbg_pkt_round, dbg_pkt_xmit,
		    dbg_pkt_handle, dbg_pkt_tcheck, dbg_pkt_epoll, dbg_pkt_batch;
int dbg_pkt_num = 0;

static inline void 
InitStatCounter(struct stat_counter *counter)
{
	counter->cnt = (int64_t)0;
	counter->sum = (int64_t)0;
	counter->max = (int64_t)0;
	counter->min = (int64_t)0;
}
/*----------------------------------------------------------------------------*/
static inline void 
UpdateStatCounter(struct stat_counter *counter, uint64_t value)
{
	counter->cnt++;
	counter->sum += value;
...
#define ROUND_DIV (1000)
	...
	fprintf(stderr, "[CPU%2d] Rounds: %4lluK, "
			...
			mtcp->ctx->cpu, (long long unsigned)rs->rounds / ROUND_DIV, 
			(long long unsigned)rs->rounds_rx / ROUND_DIV, (long long unsigned)rs->rounds_rx_try / ROUND_DIV, 
			(long long unsigned)rs->rounds_tx / ROUND_DIV, (long long unsigned)rs->rounds_tx_try / ROUND_DIV, 
			(long long unsigned)rs->rounds_select, 
			(long long unsigned)rs->rounds_select_rx, (long long unsigned)rs->rounds_select_tx, (long long unsigned)rs->rounds_select_intr);
...
PrintThreadRoundTime(mtcp_manager_t mtcp)
{
	...
	printf("[CPU%2d] Pkt_Time: (avg (cycles, max(cycles)) pkt_cnt: %d, "
		"batch: (%4lu, %4lu), " 
		"round: (%4lu, %4lu), " 
		"processing: (%4lu, %4lu), " 
		"tcheck: (%4lu, %4lu), " 
		...
		"handle: (%4lu, %4lu), " 
		"xmit: (%4lu, %4lu)\n", 
		mtcp->ctx->cpu, dbg_pkt_num,
		GetAverageStat(&dbg_pkt_batch), dbg_pkt_batch.max,
		GetAverageStat(&dbg_pkt_round), dbg_pkt_round.max,
		GetAverageStat(&dbg_pkt_proc), dbg_pkt_proc.max,
		GetAverageStat(&dbg_pkt_tcheck), dbg_pkt_tcheck.max,
		GetAverageStat(&dbg_pkt_epoll), dbg_pkt_epoll.max,
		GetAverageStat(&dbg_pkt_handle), dbg_pkt_handle.max,
		GetAverageStat(&dbg_pkt_xmit), dbg_pkt_xmit.max);
	
	InitStatCounter(&dbg_pkt_batch);
	InitStatCounter(&dbg_pkt_round);
	InitStatCounter(&dbg_pkt_proc);
	InitStatCounter(&dbg_pkt_xmit);
	InitStatCounter(&dbg_pkt_tcheck);
	InitStatCounter(&dbg_pkt_epoll);
	InitStatCounter(&dbg_pkt_handle);
}
#endif
#endif /* NETSTAT */
/*----------------------------------------------------------------------------*/
...
RunMainLoop(struct mtcp_thread_context *ctx)
{
	...
	//			   epoll_ts, handle_ts, xmit_ts;
	volatile uint64_t prev_tsc, processing_tsc, tcheck_tsc, 
				   epoll_tsc, handle_tsc, xmit_tsc,
				   start_pkt_tsc, end_pkt_tsc;
	int have_pkt = 0;
	...
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {
		...
		if (have_pkt) {
			have_pkt = 0;
			UpdateStatCounter(&dbg_pkt_round, cur_tsc - prev_tsc);
		}
		/* measure the inter-round delay */
		//UpdateStatCounter(&mtcp->rtstat.round, TimeDiffUs(&cur_ts, &prev_ts));
		UpdateStatCounter(&mtcp->rtstat.round, cur_tsc - prev_tsc);
		prev_tsc= cur_tsc;
#endif
...
#ifdef ENABLE_ONVM
		int inf_num;
		int pkt_idx[RTE_MAX_ETHPORTS] = {0};
		inf_num = g_config.mos->netdev_table->num;
		recv_cnt = mtcp->iom->recv_pkts(ctx, inf_num);
		STAT_COUNT(mtcp->runstat.rounds_rx_try);

		UpdateStatCounter(&dbg_pkt_batch, recv_cnt);
		for (i = 0; i < recv_cnt; i++) {
			uint16_t len;
			uint8_t *pktbuf;
#if TIME_STAT		
			if (have_pkt == 0) have_pkt = 1; 	
#endif
			pktbuf = mtcp->iom->get_rptr(mtcp->ctx, (long)(&rx_inf), i, &len);
			dbg_pkt_num++;
#if TIME_STAT
			start_pkt_tsc = rte_rdtsc();
#endif
			ProcessPacket(mtcp, rx_inf, pkt_idx[rx_inf], ts, pktbuf, len);

			/* update pkt index for a specific port */
			pkt_idx[rx_inf]++;
#if TIME_STAT
			end_pkt_tsc = rte_rdtsc();
			UpdateStatCounter(&dbg_pkt_proc, end_pkt_tsc - start_pkt_tsc);
#endif
		}
#else
		...
		processing_tsc = rte_rdtsc();
		UpdateStatCounter(&mtcp->rtstat.processing, processing_tsc - cur_tsc);
		//printf("update counter->cnt: %llu, sum: %llu\n", (unsigned long long)mtcp->rtstat.processing.cnt, (unsigned long long)mtcp->rtstat.processing.sum);
#endif /* TIME_STAT */
		...
		tcheck_tsc = rte_rdtsc();
		UpdateStatCounter(&mtcp->rtstat.tcheck, tcheck_tsc - processing_tsc);
		if (have_pkt) UpdateStatCounter(&dbg_pkt_tcheck, tcheck_tsc - processing_tsc);
		...
		epoll_tsc = rte_rdtsc();
		UpdateStatCounter(&mtcp->rtstat.epoll, epoll_tsc - tcheck_tsc);
		if (have_pkt) UpdateStatCounter(&dbg_pkt_epoll, epoll_tsc - tcheck_tsc);
#endif /* TIME_STAT */
		...
		handle_tsc = rte_rdtsc();
		UpdateStatCounter(&mtcp->rtstat.handle, handle_tsc - epoll_tsc);
		if (have_pkt) UpdateStatCounter(&dbg_pkt_handle, handle_tsc - epoll_tsc);
#endif /* TIME_STAT */
		...
		xmit_tsc = rte_rdtsc();
		UpdateStatCounter(&mtcp->rtstat.xmit, xmit_tsc - handle_tsc);
		if (have_pkt) UpdateStatCounter(&dbg_pkt_xmit, xmit_tsc - handle_tsc);
#endif /* TIME_STAT */
